...
function getFullNames(runners) {
  const namesList = [];
    runners.forEach(item => {
      namesList.push(`${item.last_name}, ${item.first_name}`);
    });
...
function firstNamesAllCaps(runners) {
  const namesList = runners.map(name => name.first_name.toUpperCase());
  // console.table(namesList);
  return namesList;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const shirtList = runners.filter(item => item.shirt_size == tShirtSize);
...
function tallyUpDonations(runners) {
  const a = [];
  runners.forEach(item => a.push(item.donation));
